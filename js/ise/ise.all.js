 ...
 * 4. 推送评测文本(cmd = ttp)
 * 5. 收集音频 通过websocket上传服务器
 * 6. 到达末端（用户触发 or 本地检测 ）发起音频结束指令，发起获取结果指令
 * 7. 收到结果应答，发送会话结束指令，结束会话
 */
var IFlyIatSession = (function (window, navigator) {
    ...
    var settings = {
        "serverUrl": "wss://h5.openspeech.cn/ise.do",
    ...
    var command = (function () {
        ...
        var sendIseText = function () {
        	var msg = clone(base);
            msg.cmd = "ttp";
            msg.data = {"params" : "parsed=0" , "text":iseParams.ttpText};
            socket.send(msg);
        };
    ...
    var newSocket = function (url) {
        ...
        var serverMessageHandler = (function () {
            ...
            return {
                ...
                /**
                 * 会话开始请求响应处理
                 * @param obj e.g. {"ver" : "1.0", "sub" : "ise", "cmd" : "ssb", "msg" : "response","ret":0, "data" : { "sid" : "iat@..." }}
                 */
                "onSSB": function (obj) {
                    if (obj.ret == 0) {
                ...
                /**
                 * 请求结果响应处理
                 * @param obj e.g. {"ver" : "1.0", "sub" : "ise", "cmd" : "grs", "msg" : "response","ret":0, "data" : { "sid" : "iat@..." }}
                 */
                "onGRS": function (obj) {
                ...
                /**
                 * 会话结束请求响应处理
                 * @param obj e.g. {"ver" : "1.0", "sub" : "ise", "cmd" : "sse", "msg" : "response", "ret":0,"data" : { "sid" : "iat@..." }}
                 */
                "onSSE": function (obj) {
                    sessionInfo.id = null;
                 ...
                 */
                "onXML": function(xml) {
		          	if (!isGetXmlResult) {//继续请求获取最终结果
		            	iseEvent.getResult();
		          	} else {//获取到最终结果 解析展示
		            	if (utils.validateXML(xml)) {
		            		utils.log(xml,1);
		            		iseResult = '0';
		            		iseResult = utils.parseFinalXML(xml);
		            		iseResult = JSON.stringify(iseResult);
		            		callback.onResult(0, iseResult);
                        	iseResult = "";//本次会话结束
        ...
        var onmessage = function (obj) {
    		if(utils.checkJSON(obj.data)){
		        ...
		        if (obj.data != "" ) {
		           	serverMessageHandler.onXML(obj.data);
		        } else {
		        	iseEvent.abortSession();
		        }
	        }
    ...
    var utils = {
        ...
        "validateXML" : function(xmlContent) {
		    ...
		    console.log("{msg:"+errorMessage +", error_code:"+errorCode +"}" ); 
		    return result;
        },
        ...
        "parseFinalXML" : function(xml) {
        	utils.log(xml,1);
        	var xmlDoc = utils.loadXML(xml);
        	var xmlResultElement = xmlDoc.getElementsByTagName("xml_result");
        	//xxxxxxxxxxxx
        	var ret = {
            	"totle_score": "0",
            	"totle_time": "0",
     		...
     		var result = ((xmlResultElement[0].childNodes[1]).childNodes[1]).childNodes[1];
        	ret.totle_score = (parseFloat(result.getAttribute("total_score"))*20).toFixed(2);
        	ret.totle_time = parseInt(result.getAttribute("time_len"))*10+"ms";
        	ret.content = result.getAttribute("content");
        	var sentenceArr = result.childNodes;
        	for (var h = 0; h < sentenceArr.length/2-1; h++){
        		var sentcon = sentenceArr[2*h+1].getAttribute("content");
        		if(sentcon != undefined && sentcon !="" && sentcon !="sil"){
        			var wordArr = result.childNodes[2*h+1].childNodes;
        			for (var i = 0; i < wordArr.length/2 - 1; i++) {
        				//解析word节点数据
        				var content = wordArr[2*i+1].getAttribute("content");
        				if (content != undefined && content != "" && content != "sil" && content != "silv"&& content != "fil" && content != "filv") {
        					var word = {
        						"content": content,
					 			"pinyin": wordArr[2*i+1].getAttribute("symbol"),
					 			"time": parseInt(wordArr[2*i+1].getAttribute("time_len"))*10 + "ms",
					 			"sylls":[]
							};
        					var syllArr =  wordArr[2*i+1].childNodes;
        					for (var j = 0; j < syllArr.length/2 - 1; j++) {
        						//解析syll节点数据
        						var syllcon = syllArr[2*j+1].getAttribute("content");
        						if(syllcon != undefined && syllcon != "" && syllcon != "sil" && syllcon != "silv"&& syllcon != "fil" && syllcon != "filv"){
        							var syll ={
        								"content":syllcon,
        								"pinyin":syllArr[2*j+1].getAttribute("symbol"),
        								"time":parseInt(syllArr[2*j+1].getAttribute("time_len"))*10 + "ms",
        								"phones":[]
        							};
        							//解析phone节点数据
        							var phoneArr = syllArr[2*j+1].childNodes;
        							for(var k = 0; k < phoneArr.length/2-1; k++) {
        								var phonecon = phoneArr[2*k+1].getAttribute("content");
        								if(phonecon != undefined && phonecon != "" && phonecon != "sil" && phonecon != "silv"&& phonecon != "fil" && phonecon != "filv"){
        									var phone ={
        										"content":phonecon,
        										"time":parseInt(phoneArr[2*k+1].getAttribute("time_len"))*10 + "ms",
        										"dp_message": ""
        									}
        									var dpMessage = parseInt(phoneArr[2*k+1].getAttribute("dp_message"));
        									if(dpMessage == 0){
												phone.dp_message = "正确";
        									} else if(dpMessage == 16){
        										phone.dp_message = "漏读";
        									} else if(dpMessage == 32){
        										phone.dp_message = "增读";
        									} else if(dpMessage == 64){
        										phone.dp_message = "回读";
        									} else if(dpMessage == 128){
        										phone.dp_message = "替换";
        									} else {
        								
        									}
        									syll.phones.push(phone);
        								}
        							}
        							word.sylls.push(syll);
        						}
        					}
							ret.words.push(word);
        				}	
        ...
        "checkJSON" : function (str) {
		    ...
		    try{
		        if(typeof jsondecode(str) != "object"){
		            flag = false;
		        }
		    }
		    catch(err){
    ...
    return function (setting) {
        ...
        this.start = function (ise_params_obj) {
            serverParam = ise_params_obj.params + ", rse = utf8, browser_id=" + env.browserId + ",host=" + env.domain;
            iseParams.ttpText = ise_params_obj.ise_word;
            iseParams.iseCategory = ise_params_obj.ise_category;
            iseEvent.startRecord();
        };
        /*
